...
# ------------------------- LZT Market 自动选品/购买接口 -------------------------
# 说明：
#  - 该接口用于“方案B”：不依赖外部系统动态替换 item_id，而由本服务调用 LZT Market API 自行检索并下单。
#  - 需要在请求体中提供 market_token（LZT Market 的 Bearer Token）。
#  - 通过 api_key 复用本系统的 verify_api_key 校验（默认值为 xianyu_api_secret_2024，可在系统设置中配置 qq_reply_secret_key）。

from typing import Any, Dict, Optional, List
from lzt_integration import LZTMarketClient, LztAutoBuyConfig, LztSearchRules, LZTMarketError

class LztFastBuyRequest(BaseModel):
    api_key: str
    market_token: str
    item_id: str
    attempts: int = 10
...
class LztAutoBuyRequest(BaseModel):
    api_key: str
    market_token: str
    category: str  # e.g. "steam"
    ...
    max_pages: int = 3
    per_page_delay_sec: float = 0.2
    buy_attempts: int = 10
    buy_delay_sec: float = 0.2

@app.post("/api/lzt/fast_buy")
def lzt_fast_buy(request: LztFastBuyRequest):
    if not verify_api_key(request.api_key):
        raise HTTPException(status_code=403, detail="Invalid API key")

    try:
        client = LZTMarketClient(token=request.market_token)
        resp = client.fast_buy(item_id=request.item_id, attempts=request.attempts, delay_sec=request.delay_sec)
        return {"success": True, "buy_response": resp}
    except LZTMarketError as e:
        logger.error(f"LZT fast_buy failed: {e}")
        return {"success": False, "message": str(e)}
    except Exception as e:
        logger.error(f"LZT fast_buy exception: {e}")
        return {"success": False, "message": "Internal error"}

@app.post("/api/lzt/auto_buy")
def lzt_auto_buy(request: LztAutoBuyRequest):
    if not verify_api_key(request.api_key):
        raise HTTPException(status_code=403, detail="Invalid API key")

    rules = LztSearchRules(
        include_keywords=request.include_keywords,
        any_keywords=request.any_keywords,
        exclude_keywords=request.exclude_keywords,
        min_price=request.min_price,
        max_price=request.max_price,
        seller_whitelist=request.seller_whitelist,
        seller_blacklist=request.seller_blacklist,
        min_seller_rating=request.min_seller_rating,
        min_seller_reviews=request.min_seller_reviews,
        require_in_stock=request.require_in_stock,
    )

    cfg = LztAutoBuyConfig(
        category=request.category,
        search_params=request.search_params or {},
        rules=rules,
        max_pages=request.max_pages,
        per_page_delay_sec=request.per_page_delay_sec,
        buy_attempts=request.buy_attempts,
        buy_delay_sec=request.buy_delay_sec,
    )

    try:
        client = LZTMarketClient(token=request.market_token)
        result = client.auto_buy(cfg)
        return result
    except LZTMarketError as e:
        logger.error(f"LZT auto_buy failed: {e}")
        return {"success": False, "message": str(e)}
    except Exception as e:
        logger.error(f"LZT auto_buy exception: {e}")
        return {"success": False, "message": "Internal error"}
